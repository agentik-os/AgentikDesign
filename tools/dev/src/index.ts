...
import { createSidecarLaunchEnv, requestJsonIpc } from "@agentik-design/sidecar";
import {
...
async function buildDesktop(config: ToolDevConfig, logHandle: FileHandle): Promise<void> {
  await logHandle.write(`\n[tools-dev] building @agentik-design/desktop at ${new Date().toISOString()}\n`);
  const invocation = createPackageManagerInvocation(["--filter", "@agentik-design/desktop", "build"], process.env);
...
function addPortOptions(command: ReturnType<typeof cli.command>) {
  return command
    ...
    .option("--prod", "use production build (requires pnpm --filter @agentik-design/web build first)");
}
