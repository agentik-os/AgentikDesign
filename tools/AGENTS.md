...
- `tools/dev` provides `@agentik-design/tools-dev` and the `tools-dev` bin. It is the only currently active local development lifecycle control plane.
...
- `tools/pack` provides `@agentik-design/tools-pack` and the `tools-pack` bin. The active slice is packaged artifact build/install/start/stop/logs/uninstall/cleanup/list/reset plus beta release artifact preparation for mac and Windows lanes, plus a Linux AppImage lane with optional containerized builds.
...
- Orchestration layers must consume primitives from `@agentik-design/sidecar-proto`, `@agentik-design/sidecar`, and `@agentik-design/platform`.
...
```bash
pnpm --filter @agentik-design/tools-dev typecheck
pnpm --filter @agentik-design/tools-dev build
pnpm --filter @agentik-design/tools-pack typecheck
