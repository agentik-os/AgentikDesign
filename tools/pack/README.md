...
`--headless` makes `install`, `start`, and `stop` operate on the headless entry (`@agentik-design/packaged/dist/headless.mjs`) instead of the AppImage. Headless mode runs daemon + web without Electron.
